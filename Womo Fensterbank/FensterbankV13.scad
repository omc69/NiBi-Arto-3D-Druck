...
back_steg            = 4.0;   // Materialstärke zwischen Plattenende (y=0) und ersten äußeren Löchern
...
// Hilfsmaß: innerer Bracket-Offset (wie bisher)
inner_y0 = outer_br_thickness + board_thickness + fit_clearance;
...
module plug_holes() {
    ...
    for (i = [0 : plug_count-1]) {
        x0 = plug_margin_x + i * step_x;

        // Loch für äußeren Bracket
        translate([x0, outer_y0_hole, 0])
            cube([hole_w, hole_d, hole_h], center = false);

        // Loch für inneren Bracket
        translate([x0, inner_y0_hole, 0])
            cube([hole_w, hole_d, hole_h], center = false);
    }
}
...
module plug_holes_mask_for_ridges() {
    ...
    for (i = [0 : plug_count-1]) {
        ...
        translate([x0, outer_y0_hole, top_z - 0.1])
            cube([hole_w, hole_d, mask_h], center = false);

        // Maskenblock über innerem Loch
        translate([x0, inner_y0_hole, top_z - 0.1])
            cube([hole_w, hole_d, mask_h], center = false);
    }
}
...
// Innerer Bracket in Einbaukoordinaten (wie bisher)
